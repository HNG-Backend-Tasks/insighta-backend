...
Query results are cached in-process using a TTLCache. Repeated queries with identical filters — regardless of how they were expressed — are served from cache without hitting the database.
...
| Role | Permissions |
|---|---|
| admin | Create profiles, delete profiles, read, search, export, import CSV |
...
All profile list and search queries are cached in-process using a TTLCache (5 minute TTL, 1000 entry max).

Cache keys are generated by:
1. Parsing the query into a filter dict
2. Serializing with `sort_keys=True` — guarantees consistent ordering
3. MD5 hashing the result into a fixed-length key

This means semantically identical queries like `"young males from nigeria"` and passing `min_age=16&max_age=24&country_id=NG` directly produce the same cache key and hit the same cached result.
...
Admins can upload CSV files containing up to 500,000 profiles via `POST /api/profiles/import`.

- File is streamed — never loaded fully into memory
- Processed in chunks of 1000 rows — minimal DB round trips
- Existing profile names loaded into memory at upload start for O(1) duplicate checks
- Concurrent uploads supported — INSERT OR IGNORE prevents constraint violations
- Bad rows are skipped, never fail the entire upload
...
| POST | `/api/profiles` | admin | Create single profile |
| POST | `/api/profiles/import` | admin | Bulk import via CSV upload |
...
To verify cache effectiveness against a running server:

```bash
uv run python scripts/benchmark.py
```

This measures DB queries fired vs cache hits across identical and varied query patterns.
